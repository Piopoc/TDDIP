...
import com.example.demo.util.Extractor;
import com.example.demo.util.JsonlEnhancer;
...
public class DocumentController {
    ...
    public ResponseEntity<String> importFromPipeline() {
        try {
            Extractor.extractAll("GB");

            String stoplist = "dataset/data/stoplist_en.txt";
            Set<String> stopwords = TextCleaner.loadStopwords(stoplist);

            TextCleaner.processJsonl("dataset/data/mediabias_newsarticles.jsonl", "dataset/output/cleaned_articles.jsonl", stopwords);
            TextCleaner.processJsonl("dataset/data/parlamint_raw.jsonl", "dataset/output/cleaned_parlamint.jsonl", stopwords);

            JsonlEnhancer.processFile("dataset/output/cleaned_articles.jsonl", "dataset/output/enhanced_articles.jsonl");
            JsonlEnhancer.processFile("dataset/output/cleaned_parlamint.jsonl", "dataset/output/enhanced_parlamint.jsonl");

            fetchService.importDocumentsFromPipeline();

            return ResponseEntity.ok("Pipeline completata con successo!");
        } catch (Exception e) {
            return ResponseEntity.internalServerError().body("Errole nella pipeline: " + e.getMessage());
        }
    }
